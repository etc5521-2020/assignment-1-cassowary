...
# I used str_split() as suggested by Mitchell O’Hara-Wild
# I later found that separate rows seems easier
# I stole from stackoverflow anyway: 
# https://stackoverflow.com/questions/30283873/how-to-split-one-row-into-multiple-rows-in-r

gdpr_vio_art <- as.tibble(gdpr_vio$article_violated)

gdpr_vio_art_list <- gdpr_vio_art %>%  separate_rows(value, sep = "\\|")
# gdpr_vio_art_list <- tibble(value = unlist(strsplit(gdpr_vio$article_violated, "\\|")))

```

```{r art-plot}
# Prepare clean data to make plot.
# I noticed that what I need is the first 7 characters.
# Then I trim the character out, leaving only digit.

gdpr_vio_art_list <- gdpr_vio_art_list %>% 
  mutate(article = (substr(value, start = 1, stop = 7))) %>%
      mutate(article_number = as.numeric(gsub("\\D+"," ", article)), # Trim character out
             article_name = paste("Article.", article_number)) %>%   # Add article text, for beauty
  na.omit() %>%
  group_by(article_number) %>%
  mutate(count = n()) # For reorder of group

# gdpr_vio_art_plot 
gdpr_vio_art_list %>%
  ggplot()+
  geom_bar(aes(y = fct_reorder(article_name, count))) +
  xlab("Article")
```
