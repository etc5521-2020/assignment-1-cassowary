...
Figure \@ref(fig:artfines) shows us a violation of Article 5, 6 and 32 will lead to a relatively large fine, and with a high chance, the amount of the fine will falls between 10000-100000 euros. 
...
## Analysis of violations by Country

In this part we will be looking into the number of violations made in each country according to the data set. 

```{r}
country <- gdpr_vio %>% select(name) #%>%group_by(name) %>% count() %>% arrange(n)#%>% filter(n > 1)
```

```{r country_violation_count}
cc <- country %>% ggplot() +  
...
Figure \@ref(fig:country_violation_count) shows that Spain, standing at 100 violations; has had the most recorded violations from all the countries in the data set. 
