...
plot1 <- gdpr_vio %>%
  ...
  geom_text(data = filter(filter(gdpr_vio, date>="2019-12-01"&date<="2020-04-01"), 
                           price > 4e+05),
             aes(factor(month(date)), price, label = name), size = 3.5) + 
  geom_text(data = filter(filter(gdpr_vio, date>="2019-10-01"&date<="2019-11-30"), 
                           price > 1e+06),
             aes(factor(month(date)), price, label = name), size = 3.5) +
...
Figure \@ref(fig:plot4) demonstrates the number of fined GDPR violations by country from 2018 to 2020. While all country in the EU has less than 35 cases of GDPR violations except for Spain, which has nearly 100 cases, triple than that of Hungary, the second highest. It is an interesting trend that should be explored by further research. 
```{r plot4, fig.cap = "Number of GDPR violations by country, 2018-2020"}
temp <- gdpr_vio %>%
  .[['name']] %>%
  unique()
temp <- ne_countries(returnclass = "sf", country = temp)
temp <- gdpr_vio %>%
  group_by(name) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  right_join(temp, by = c("name" = "sovereignt"))
p <- ggplot() +
  geom_sf(data = temp, aes(geometry = geometry, fill = count, label = name)) +
  coord_sf(xlim = c(-25,30), ylim = c(30,82)) +
...
Table \@ref(tab:table1) shows us the information about the ten corporations that have accumulated the most fines since the introduction of GDPR. `r table1$controller[1]` was fined `r table1$count[1]` times and had to pay a fortune of `r table1$total_price[1]/1000000` million euros. It is worth notice that VodaFone was fined 19 times, the highest number of financial penalties received. A closer look at the Britist telecommunication company is provided in table \@ref(tab:table2). We discovered another fact: Not only did Vodafone España	receive a very high number of fines, its branch, Vodafone ONO also receive 2 fines. Together, Vodafone violated the GDPR 21 times in Spain, which is a fifth of the country's total number of violations.
...
Ignore these highly complicated article phrases, we can still find some interesting terms we are familiar with. Like the one "historical research purpose". It suggests data analysis is under consideration of GDPR.
...
The reports also explored the trends in financial penalties received by data controller. **Need more detail**
