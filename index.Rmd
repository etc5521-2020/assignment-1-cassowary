...
gdpr_vio <- mutate(gdpr_vio, price = if_else(price <= 0, NA_real_, price)) %>%
  ...
  mutate(controller = if_else(tolower(controller) == "unknown company", "unknown", controller)) %>%
  mutate(controller = if_else(tolower(controller) %in% c("unknown", "unknwon"), "unknown", controller)) %>%
