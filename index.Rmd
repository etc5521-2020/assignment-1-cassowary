...
Meanwhile, @eufuture suggested that the EU was in a bad position on the battle to its digital future. Besides, part of the reasons behind the GDPR establishment and the later digital services taxes imposed by major EU countries were an indication that the EU was waving the flag of digital sovereignty. Thus, against digital hegemony, it intentionally targeted non-local held IT corporations.
...
- The controller of data, in many cases, are vaguely described, or not disclosed. This trend can lead to misunderstanding and bias in further analysis.

The data consist of two smaller datasets:

1. `GDPR_Violations.tsv` contains information of `r nrow(gdpr_vio)` fines and penalties which data protection authorities in the EU have imposed under EU General Data Protection Regulation (GDPR). The dataset contains variables including the country and authority, the date of violation, the fine price of violation in Euros, the controller that violate the rule, ... Detailed information of variable is presented below.
...
3. What were the keywords in each article, especially articles referenced the most in GDPR fines? How did it reflect the privacy issues in the EU?
...
Figure \@ref(fig:plot1) shows the price of GDPR fines from 2018-2020. We can notice the high price clustering (dots in red) between Sep 2019 and Apr 2020. These outliers include Poland, Austria, Germany, Italy, Sweden and the Netherlands. If we further research on the spatial patterns of these outliers in Figure \@ref(fig:outliers), we can see that these countries are all connected. We suspect there are political reasons behind this phenomenon, which is worth for future study. Other than that, the reason we use a log 10 scale of the price is that price is an extremely skewed distribution. Using the log scale of the price can provide us with better data representation.
...
Figure \@ref(fig:plot2) shows the number of GDPR violations that got fined from 2018 to 2020. Time periods when a high number of GDPR fines were recorded are painted in red in order to highlight the possible trend. The last quarter of 2019 and the first quarter of 2020 saw the highest number of GDPR violations, with a peak of 35 controllers fined in March 2020.
...
Figure \@ref(fig:plot4) demonstrates the number of fined GDPR violations by country from 2018 to 2020. While all country in the EU has less than 35 cases of GDPR violations except for Spain, which has nearly 100 cases, triple than that of Hungary, the second-highest. This is a noticeable trend that requires further research. 
...
In this part we want to look at are there taxes involved in articles that are there for regulating policies on Data. 

```{r interacttable, eval=FALSE}

#could not get the tables to show in the html
words <- word_list %>% select(trigram)

words <- words %>% separate(trigram, c("A", "B", "C"))

#dt <-  
datatable(words, options = list(pageLength = 7), class = 'cell-border stripe', filter = 'top')


#htmlwidgets::createWidget(words, dt)

#dt <- frameWidget(dt, width = "100%")
#frameWidget(datatable(words, options = list(pageLength = 7), class = 'cell-border stripe', filter = 'top'))
...
```{r interactive_table, eval=FALSE}

#could not get the tables to show up in the html
...
#In the Table \@ref(tab:interactive_table), if we look for taxes in the search bar nothing shows up, hence, suggesting there are no tax regulations in the articles, according to the data set. However, the table is not limited to taxes it can be used to explore other keywords to identify which articles cover those grounds, or maybe of the user's interests. Try it, try looking up "criminal", or "sex".
```

Using the dataset for the word clouds, a deep analysis was conducted in the articles to look into the textual composition of the articles, if they regulated any types of tax or taxes on data. Upon exploration of the dataset, by making substrings, it was found that no tax regulations are in place under the articles from the dataset. Therefore, it can be said that there are only fines listed in the General Data Protection Regulations. 
...
The reports find that since the introduction of GDPR, the period from October 2019 to April 2020 saw the largest number of GDPR violations. Overall, the most country has less than 30 violations, except for Spain. The country recorded about 100 GDPR financial penalties, 21 of which from Vodafone.

Regarding the General Data Protection Regulation, the report discovers that the most commonly violated GDPR's article is article 5, article 6 and article 32, All of which focused on the "Processing" of personal data.  It indicates that the "Processing" of personal data in the EU may have issues.
...
Another research field that can be focused on is the issues in the "Processing" of personal data in the EU. As the most commonly violated GDPR's article are all related to Processing of personal data, there seem to be issued in this field.
