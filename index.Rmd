...
Available data of GDPR fines from @Privacy-Affairs, provides an opportunity for us to gain insight into the privacy issues and possible abuse of articles of GDPR in EU. This dataset was originally adopted by @Tidy-Tuesday, which is a social weekly data project in R. [github address here](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-04-21/readme.md) We outline the workflow of data collection, data cleaning and data analysis in the following sections. 

# Limitations
A potential limitation of this research is a great proportion of controller names(about 10%) are missing from the data set which introduces extra difficulties to draw accurate conclusions with respect to the company nationality and identity. 

Fines documented by @Privacy-Affairs pertains to fines made public hence many of the fines may not be present. We might think of this data set as "high-profile" fines. 
...
The data is scraped from @Privacy-Affairs by @Ellis-Hughes, posted on @Tidy-Tuesday. We adapted the scripts they used and removed duplication of GDPR fines due to multiple sources. We looked at data spanning from 2018-05-12 to 2020-08-21. 

Abnormal values including fines with 0 price, indicating ongoing trails and fines before the establishment of the GDPR (which are missing values) are set NA. Records with missing date are also set to NA. We further process inconsistency in corporation names. There are a number of unnamed organizations fined by GDPR. in the data set. These are labelled as "Unknown". 
...
- The controller of data, in many cases, are vaguely described, or not disclosed. This may lead to misunderstanding and bias in our analysis.

The data consist of two datasets:

1. `GDPR_Violations.tsv` contains information of `r nrow(gdpr_vio)` fines/penalties which data protection authorities (in EU) have imposed under GDPR. The dataset contains variables includes the country(where violation enforced) and authority, date of violation, fine price of violation in Euros. 
...
The overall aim of this research is to examine the demographics in the application of the GDPR in practice since its implementation (in 25 May 2018) and unveil prevalent privacy issues in cyberspace in the EU.
...
Questions  
4. Do large companies and small companies tend to violate the same articles as smaller companies?  
5. Expanding original report: Looking into high clustering behind spade of exorbitant fines given in span of Sep 2019 to Apr 2020  
6. Expanding original report: follow up why Spain dished out most fines from 2018-2020 as per Figure \@ref(fig:plot4))  

Improvements
- distinguish developed & developing countries; in Boxplot of GDPR fines by country in Figure \@ref(fig:plot1)
- Enhance reproducibility; by adding comments 
...
Figure \@ref(fig:plot2) shows the number of GDPR fines dished out in for each month for years 2018-2020. Time periods when high number of GDPR fines were recorded are highlighted red. The last quarter of 2019 and the first quarter of 2020 saw the highest number of GDPR violations, with a peak of 35 controllers fined in March 2020.

Figure \@ref(fig:plot1) shows log price of GDPR fines against month for 2018-2020. We notice high price clustering (dots in red) between Sep 2019 and Apr 2020.  
log price was used because price is an extremely skewed distribution. Using log scale of the price provide us with better data plot.

These outliers include Poland, Austria, Germany, Italy, Sweden and Netherlands. Looking at the spatial patterns of these outliers in Figure \@ref(fig:outliers), we see countries are all closely connected to one other. We suspect there could be political reasons which could be worth the further study.
...
```
Figure \@ref(fig:map) depicts accumulated fines per country. Accumulated fines appear to be concentrated in the middle where populations are dense. 
...
Boxplot in Figure \@ref(fig:plot3) is shaded according to the price categories in \@ref(fig:map). It is not necessarily that a country with the highest accumulated fines(in absolute € terms) has the most number of price dished out. For example, Netherlands has €3.49M in accumulated fines but has the highest median in fine price.
...
Figure \@ref(fig:plot4) demonstrates the number of fined GDPR violations by country from 2018 to 2020. All country in the EU has less than 35 cases of GDPR fines except for Spain, which has nearly 100 cases. Some exorbitant fines can be seen such as France which accumulated 51M in 5 fines. 
